...
public class Pipes {
    ...
    private float DISTANCE_BETWEEN_PIPES = 420f;
    ...
    void createPipes(float x, float y){
        ...
        bodyDef.position.set(pipe1.getX() / GameInfo.PPM,
                pipe1.getY() / GameInfo.PPM);

        body1 = world.createBody(bodyDef);
        body1.setFixedRotation(false);

        bodyDef.position.set(pipe2.getX() / GameInfo.PPM,
                pipe2.getY() / GameInfo.PPM);
    ...
    float getRandomY(){
        float max = GameInfo.HIGHT / 2f + 150f;
        float min = GameInfo.HIGHT / 2f - 150f;
